...
public function url_practil_registrar_user($nombre,$apellido,$password,$email,$tipo_registro){
      ...
      $servicio_practil = $servicio_practil.'nombre='.urlencode($nombre).'&';
      $servicio_practil = $servicio_practil.'apellido='.urlencode($apellido).'&';
      $servicio_practil = $servicio_practil.'password='.urlencode($password).'&';
      $servicio_practil = $servicio_practil.'email='.urlencode($email).'&';
      $servicio_practil = $servicio_practil.'plataforma='.urlencode($this->return_pk_plataforma()).'&';
      $servicio_practil = $servicio_practil.'tipo_registro='.urlencode($tipo_registro).'&';
      $servicio_practil = $servicio_practil.'token='.urlencode($this->return_token_aplication());
...
public function url_practil_confirmation_account($email,$token_acount){
      ...
      $servicio_practil = $servicio_practil.'email='.urlencode($email).'&';
      $servicio_practil = $servicio_practil.'token_account='.urlencode($token_acount).'&';
      $servicio_practil = $servicio_practil.'token_platform='.urlencode($this->return_token_aplication());
      return $servicio_practil;
}
...
public function url_practil_login($email,$password){
      ...
      $servicio_practil = $servicio_practil.'email='.urlencode($email).'&';
      $servicio_practil = $servicio_practil.'password='.urlencode(md5($password)).'&';
      $servicio_practil = $servicio_practil.'plataforma='.urlencode($this->return_pk_plataforma()).'&';
      $servicio_practil = $servicio_practil.'token_platform='.urlencode($this->return_token_aplication());
      return $servicio_practil;
}
...
public function url_practil_get_users($user_ids=array()){
    $servicio_practil = "";
    $servicio_practil .= sfConfig::get('app_url_practil').$this->return_module_aplication('json').'get_users_by_pk_in_array?';
    $servicio_practil .= 'token='.urlencode($this->return_token_aplication());
    foreach($user_ids as $user_id){
        $servicio_practil .= '&list_pk[]='.$user_id;
    }
    return $servicio_practil;
}
