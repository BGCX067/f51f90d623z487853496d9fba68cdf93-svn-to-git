...
ECOTree = function (obj, elm) {
	this.config = {
		...
		expandedImage : 'http://humanscorecard.practil.com/images/less.gif',
		collapsedImage : 'http://humanscorecard.practil.com/images/plus.gif',
		transImage : 'http://humanscorecard.practil.com/images/trans.gif'
	}
		...
		if (!node._isAncestorCollapsed())
		{
			switch (this.render)
			{
				case "CANVAS":
					...
					//HTML part...
					s.push('<div id="' + node.id + '" class="econode scordpractil" style="{top:'+(node.YPosition+this.canvasoffsetTop)+'; left:'+(node.XPosition+this.canvasoffsetLeft)+'; width:'+node.w+'; height:'+node.h+';}" ');
					...
					if (node.canCollapse) {
						s.push('<a style="position:absolute;z-index:500;" id="c' + node.id + '" href="javascript:'+this.obj+'.collapseNode(\''+node.id+'\', true);" >');
...
ECOTree.prototype.toString = function () {	
	...
	{
		case "CANVAS":
			s.push('<canvas id="ECOTreecanvas" class="canvas-style" width="300px" height="300px"></canvas>');
			break;
...
ECOTree.prototype.UpdateTree = function () {	
	...
	if (this.render == "CANVAS") {
		var canvas = document.getElementById("ECOTreecanvas");
                canvas.setAttribute('width',this.config.canvasWidth);
                canvas.setAttribute('height',this.config.canvasHeight);
